Shader "Custom/HologramShader" {
	...
	SubShader {
		...
		void surf (Input IN, inout SurfaceOutputStandard o) {
			...
			clip(frac((IN.uv_MainTex.y * 10 - time * 50.1) * 5) - 0.5);
			clip(frac((IN.uv_MainTex.y * 40 - time * 60.15) * 5) - 0.4);
			//fixed4 c = tex2D(_MainTex, float2(IN.uv_MainTex.x - time * 0.5, IN.uv_MainTex.y + sin(IN.uv_MainTex.x + time * 1.0) * 0.15)) * _Color;
			fixed4 c = tex2D(_MainTex, float2(IN.uv_MainTex.x + sin(time + IN.uv_MainTex.y * 1000) * 0.007, IN.uv_MainTex.y + sin(IN.uv_MainTex.x + time * 1.0) * 0.01)) * _Color;
			o.Albedo = c.rgb;
			// Metallic and smoothness come from slider variables
			o.Metallic = _Metallic;
			o.Smoothness = _Glossiness;
			o.Alpha = _Color.a * c.a;
			float val = IN.uv_MainTex.y + sin(IN.uv_MainTex.x + time * 1.0) * 0.01;
			//if (val > 0.75) val = -1;
			//if (val < 0.25) val = -1;
			if (o.Alpha == 0) val = -1;
			clip(val);
